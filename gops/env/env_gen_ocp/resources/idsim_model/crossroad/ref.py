...
def get_ref_param(env: CrossRoad, model_config: ModelConfig, light_param: np.ndarray) -> np.ndarray:
    ...
    for ref_line in ref_list:
        ...
        if current_part['destination'] == True:
            intervals, ref_v = compute_intervals_destination(
                num_ref_points + N - 1, ref_v_lane, dt)
...
# def compute_intervals_green(position_on_ref: float,
#                       ref_info: List[Dict[str, float]],
#                       total_num: int,
#                       ref_v_lane: float,
#                       ref_v_junction: float,
#                       dt: float,
...
#     position_on_ref = np.clip(position_on_ref, a_min=0,
#                               a_max=current_part['length'])
#     # assert position_on_ref <= current_part['length'], (position_on_ref, current_part['length'])
#     if current_part['destination'] == True:
#         # ref_v keeps 8.0
#         intervals[:] = ref_v_lane * dt
#         ref_v[:] = ref_v_lane
...
# def compute_intervals_red(position_on_ref: float,
#                       ref_info: List[Dict[str, float]],
#                       total_num: int,
#                       ref_v_lane: float,
#                       dt: float,
...
#                               a_max=current_part['length'])
#     # assert position_on_ref <= current_part['length'], (position_on_ref, current_part['length'])
#     if current_part["in_junction"] == False and current_part['destination'] == False:
#         # ref_v from ref_v_lane to ref_v_junction
#         ahead_length = current_part['length'] - position_on_ref - min_ahead_lane_length
#         if ahead_length < 0:
#             intervals = np.zeros((total_num, ))
#             ref_v = np.zeros((total_num, ))
#         else:
...
#             intervals = np.clip(
#                 intervals, a_min=0 * dt, a_max=ref_v_lane * dt)
#     elif current_part['destination'] == True or (current_part['destination'] == False and current_part["in_junction"] == True):
#         raise ValueError("Error path planning for red mode")
#     else:
#         raise ValueError("Error ref_line")
#     # repeat the last v
#     ref_v = np.append(ref_v, ref_v[-1])
...
#     # only update ref_v for red mode
#     traffic_light, ahead_lane_length = light_param[0, 0], light_param[0, 1]
#     N = model_config.N
#     num_ref_lines = model_config.num_ref_lines
#     num_ref_points = model_config.num_ref_points
#     max_ahead_lane_length = model_config.ahead_lane_length_max
#     min_ahead_lane_length = model_config.ahead_lane_length_min
#     ref_v = model_config.ref_v_lane
#     dt = env.config.dt
#     if traffic_light == 0 \
#         or ahead_lane_length > max_ahead_lane_length \
#         or ahead_lane_length == -1:
#         # green mode
#         pass
#     else:  # red
#         if ahead_lane_length < min_ahead_lane_length:
#             ref_v_param = np.ones((num_ref_lines, num_ref_points + N, 1)) * 0.
#         else:
#             decel = ref_v ** 2 / 2 / (max_ahead_lane_length - min_ahead_lane_length)
#             current_ref_v = np.sqrt(2 * decel * (ahead_lane_length - min_ahead_lane_length))
#             decel_process = np.arange(0, num_ref_points + N, 1) * decel * dt
#             decel_array = np.tile(decel_process, num_ref_lines).reshape(num_ref_lines, num_ref_points + N, 1)
#             ref_v_param = np.clip(current_ref_v - decel_array, a_min=0, a_max=ref_v)
#         ref_param[:, :, -1] = ref_v_param[..., -1]
#     return ref_param